...
impl Smallvil {
    ...
    pub fn relayout_tiles(&mut self) {
        ...
        let stack_count = total - 1;
        let master_width = if total == 1 {
            output_geo.size.w
        } else {
            ((output_geo.size.w as f32) * 0.6) as i32
        };
        let stack_width = output_geo.size.w - master_width;

        for (index, window) in windows.iter().enumerate() {
            let idx = index as i32;
            let (x, y, w, h) = if idx == 0 {
                (
                    output_geo.loc.x,
                    output_geo.loc.y,
                    master_width,
                ...
                let slot_h = (output_geo.size.h / stack_count).max(1);
                let y = output_geo.loc.y + slot_h * (idx - 1);
                let h = if idx == total - 1 {
                    output_geo.size.h - slot_h * (stack_count - 1)
                } else {
                    slot_h
                };
                (output_geo.loc.x + master_width, y, stack_width, h)
            };
            ...
            if let Some(toplevel) = window.toplevel() {
                toplevel.with_pending_state(|state| {
                    state.size = Some((w.max(1), h.max(1)).into());
                });
